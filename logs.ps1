# Get API key: https://ipgeolocation.io/
$API_KEY      = ""
$LOGFILE_NAME = "failed_rdp.log"
$LOGFILE_PATH = "C:\ProgramData\$($LOGFILE_NAME)"

# Check for failed RDP attempts from Windows Event Viewer
$XMLFilter = @'
<QueryList> 
...
# Train the Extract feature in Log Analytics workspace.
...
# Keep checking the Event Viewer logs.
while ($true)
{
    
    Start-Sleep -Seconds 1
    # Retrieve events from Windows Event Viewer based on the earlier created filter
    $events = Get-WinEvent -FilterXml $XMLFilter -ErrorAction SilentlyContinue
    if ($Error) {
        #Write-Host "No Failed Logons found. Re-run script when a login has failed."
    }

    # Step through each event collected, get geolocation, add to custom log
    foreach ($event in $events) {
        ...
        if ($event.properties[19].Value.Length -ge 5) {
            ...
            $eventId = $event.Id
            $destinationHost = $event.MachineName # Destination
            $username = $event.properties[5].Value # Attempted username
            $sourceHost = $event.properties[11].Value # Source
            $sourceIp = $event.properties[19].Value # Source IP Address
            ...
            if (-Not ($log_contents -match "$($timestamp)") -or ($log_contents.Length -eq 0)) {
                ...
                # Pause for a second as to not rate-limit the API
                Start-Sleep -Seconds 1
                ...
                #
                "latitude:$($lat),longitude:$($long),destinationhost:$($destinationHost),username:$($username),sourcehost:$($sourceIp),state:$($state_prov), country:$($country),label:$($country) - $($sourceIp),timestamp:$($timestamp)" | Out-File $LOGFILE_PATH -Append -Encoding utf8

                Write-Host -BackgroundColor Black -ForegroundColor Magenta "latitude:$($lat),longitude:$($long),destinationhost:$($destinationHost),username:$($username),sourcehost:$($sourceIp),state:$($state_prov),label:$($country) - $($sourceIp),timestamp:$($timestamp)"
            }
            else {
                # Entry already exists in custom log file. Do nothing.
            }
        }
    }
}
